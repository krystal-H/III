...
const originCount = [
    ...
    { label: '新增APP用户次日留存数(昨日)', count: '--' },
    { label: '活跃APP用户次日留存数(昨日)', count: '--' },
...
const columns = [
    ...
    {
        title: '新增APP用户次日留存数(昨日)',
        dataIndex: 'newRemainNum',
        key: 'newRemainNum',
    },
    {
        title: '活跃APP用户次日留存数(昨日)',
        dataIndex: 'activeRemainNum',
        key: 'activeRemainNum',
    },
    {
        title: '累计APP用户数',
...
export default function Device() {
    ...
    const dealCount = (origin) => {
        let count = [
            ...
            { label: '新增APP用户次日留存数(昨日)', count: origin.newRemainNum || 0 },
            { label: '活跃APP用户次日留存数(昨日)', count: origin.activeRemainNum || 0 },
    ...
    const dealData = (data) => {
        ...
        data.forEach(item => {
            ...
            if (currentTab == 0) {
                ...
                xData.push(item.newRemainNum)
            } else if (currentTab == 3) {
                xData.push(item.activeRemainNum)
            }
